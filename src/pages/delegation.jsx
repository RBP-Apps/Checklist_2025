...
function DelegationDataPage() {
  ...
  // Get unique Given By values from the current data
  const givenByList = useMemo(() => {
    const uniqueGivenBy = new Set()
    accountData.forEach((item) => {
      if (item["col3"]) {
        uniqueGivenBy.add(item["col3"])
      }
    })
    return Array.from(uniqueGivenBy).sort()
  }, [accountData])

  // Get unique Name values from the current data
  const nameList = useMemo(() => {
    const uniqueNames = new Set()
    accountData.forEach((item) => {
      if (item["col4"]) {
        uniqueNames.add(item["col4"])
      }
    })
    return Array.from(uniqueNames).sort()
  }, [accountData])

  // Enhanced filtered data with frontend filters
  ...
  }, [accountData, debouncedSearchTerm, selectedGivenBy, selectedNames, sortDateWise])

  // Enhanced history filtering with user filter based on column H
  const filteredHistoryData = useMemo(() => {
    return historyData
      .filter((item) => {
        ...
        const matchesGivenBy = selectedGivenBy.length > 0 ? selectedGivenBy.includes(item["col9"]) : true

        const matchesName = selectedNames.length > 0 ? selectedNames.includes(item["col7"]) : true
        ...
        return matchesSearch && matchesGivenBy && matchesName && matchesDateRange
      })
      .sort((a, b) => {
  ...
  }, [historyData, debouncedSearchTerm, selectedGivenBy, selectedNames, startDate, endDate, parseDateFromDDMMYYYY, userRole, username])
            ...
            <>
              ...
              <div className="p-4 border-b border-purple-100 bg-gray-50">
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                  ...
                  {(selectedGivenBy.length > 0 || selectedNames.length > 0 || startDate || endDate || searchTerm) && (
                    ...
                    {filteredHistoryData.length > 0 ? (
                      ...
                      <tr>
                        <td colSpan={userRole === "admin" ? 9 : 8} className="px-6 py-4 text-center text-gray-500">
                          {searchTerm || selectedGivenBy.length > 0 || selectedNames.length > 0 || startDate || endDate
